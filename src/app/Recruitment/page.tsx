...
import { useScrollManager } from "../context/ScrollContext"; // Import ScrollContext
...
const RecruitmentForm: React.FC = () => {
  ...
  const { navigateToPage, animationDirection, setAnimationDirection } = useScrollManager(); // Use ScrollContext
  ...
  const onInvalid = (errors: any) => {
    ...
    // Scroll to preferences section if there’s an error there
    if (errors.preference1 || errors.preference2) {
      setAnimationDirection("forward");
      navigateToPage("preferences-section");
    }
  };
  ...
  // Enhanced scrollbar styling for preference dropdowns
  const preferenceSelectContentStyle = "min-w-[var(--radix-select-trigger-width)] max-h-[200px] bg-neutral-900/95 backdrop-blur-md border border-neutral-700 text-white rounded-xl shadow-lg custom-select-scrollbar overflow-y-auto";
  const selectContentStyle = "min-w-[var(--radix-select-trigger-width)] max-h-[300px] overflow-y-auto bg-neutral-900/80 backdrop-blur-md border border-neutral-700 text-white rounded-xl shadow-lg custom-select-scrollbar";
    ...
    <div className="min-h-screen relative bg-black flex flex-col items-center justify-center px-4 py-12 overflow-hidden">
        ...
        <div className="absolute top-4 right-4 sm:top-6 sm:right-6 z-10 max-w-[60vw] sm:max-w-none">
          <div className="px-0 py-[2px] rounded-full bg-gradient-to-b from-neutral-500 to-neutral-800">
            <Button variant="secondary" className="rounded-full px-5 sm:px-5 lg:px-5 py-2 sm:py-3 lg:py-4 bg-neutral-900 text-white border border-neutral-800 hover:bg-neutral-800 transition text-[10px] sm:text-sm md:text-base whitespace-nowrap max-w-full truncate" onClick={() => window.open("https://aaruush.org", "_blank")}>
              Visit Our Website
            </Button>
          </div>
        </div>
        ...
        <div className="w-full max-w-[780px] mt-4 z-10 px-2 sm:px-0">
          <div className="py-px rounded-2xl bg-gradient-to-b from-[#796856] to-[#574030a1]">
            <div className="bg-[#2a170b] rounded-2xl px-3 sm:px-6 py-2.5 sm:py-3.5 text-sm sm:text-md text-neutral-200 font-medium tracking-normal">
              ...
              <span className="ml-1.5"> The registration form for Team Envision has to be filled separately. To fill{" "}
                <button onClick={handleClick} className="text-orange-500 underline font-semibold cursor-pointer hover:text-orange-400 transition-colors">Click Here.</button>
              </span>
            </div>
          </div>
        </div>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit, onInvalid)} className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-6 sm:gap-x-12 mt-8 w-full max-w-5xl z-10 px-2 sm:px-0">
            ...
            <FormField control={form.control} name="name" render={({ field }) => (
              <FormItem><FormLabel>Name</FormLabel><div className={gradientWrapperStyle}><FormControl><Input placeholder="Enter your name" className={innerFieldStyle} {...field} /></FormControl></div><FormMessage /></FormItem>
            )}/>
            <FormField control={form.control} name="regNumber" render={({ field }) => (
              <FormItem><FormLabel>Registration Number</FormLabel><div className={gradientWrapperStyle}><FormControl><Input placeholder="Enter your registration number" className={innerFieldStyle} {...field} /></FormControl></div><FormMessage /></FormItem>
            )}/>
            <FormField control={form.control} name="branch" render={({ field }) => (
              <FormItem><FormLabel>Branch</FormLabel><div className={gradientWrapperStyle}><FormControl><Input placeholder="Enter your branch" className={innerFieldStyle} {...field} /></FormControl></div><FormMessage /></FormItem>
            )}/>
            <FormField control={form.control} name="year" render={({ field }) => (
              <FormItem><FormLabel>Year</FormLabel><div className={gradientWrapperStyle}><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger className={innerFieldStyle}><SelectValue placeholder="Select Year" /></SelectTrigger></FormControl><SelectContent className={selectContentStyle}>{yearOptions.map(opt => <SelectItem key={opt.value} value={opt.value} className={selectItemStyle}>{opt.label}</SelectItem>)}</SelectContent></Select></div><FormMessage /></FormItem>
            )}/>
            <FormField control={form.control} name="email" render={({ field }) => (
              <FormItem><FormLabel>Personal Email</FormLabel><div className={gradientWrapperStyle}><FormControl><Input placeholder="Enter your email" className={innerFieldStyle} {...field} /></FormControl></div><FormMessage /></FormItem>
            )}/>
            <FormField control={form.control} name="srmEmail" render={({ field }) => (
              <FormItem><FormLabel>SRM Email</FormLabel><div className={gradientWrapperStyle}><FormControl><Input placeholder="Enter your SRM email" className={innerFieldStyle} {...field} /></FormControl></div><FormMessage /></FormItem>
            )}/>
            <FormField control={form.control} name="phoneNumber" render={({ field }) => (
              <FormItem><FormLabel>Phone Number</FormLabel><div className={gradientWrapperStyle}><FormControl><Input placeholder="Enter your phone number" className={innerFieldStyle} {...field} /></FormControl></div><FormMessage /></FormItem>
            )}/>
            <FormField control={form.control} name="whatsappNumber" render={({ field }) => (
              <FormItem><FormLabel>WhatsApp Number</FormLabel><div className={gradientWrapperStyle}><FormControl><Input placeholder="Enter your WhatsApp number" className={innerFieldStyle} {...field} /></FormControl></div><FormMessage /></FormItem>
            )}/>

            {/* Preferences Section with ID for ScrollContext */}
            <div id="preferences-section" className="col-span-1 sm:col-span-2">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-6 sm:gap-x-12">
                <FormField control={form.control} name="preference1" render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-white font-medium">Preference 1</FormLabel>
                    <div className={gradientWrapperStyle}>
                      <Select onValueChange={field.onChange} value={field.value}>
                        ...
                        <SelectContent className={`${preferenceSelectContentStyle} preference-select-content`} style={{ overflowY: 'auto', paddingRight: '12px', boxSizing: 'border-box' }}>
                          {initialGroupedOptions.map(group => (
                            <SelectGroup key={group.label}>
                              <SelectLabel className="text-neutral-400 px-2 py-1.5 font-medium">{group.label}</SelectLabel>
                ...
                <FormField control={form.control} name="preference2" render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-white font-medium">Preference 2</FormLabel>
                    <div className={gradientWrapperStyle}>
                      <Select onValueChange={field.onChange} value={field.value}>
