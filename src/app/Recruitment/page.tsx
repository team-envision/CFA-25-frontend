...
const yearOptions = [{ value: "1", label: "1st Year" }, { value: "2", label: "2nd Year" }, { value: "3", label: "3rd Year" }];
const allGroupedOptions = [{ label: "Teams", options: teamsOptions }, { label: "Committees", options: committeesOptions }, { label: "Domains", options: domainsOptions }];
  ...
  return (
    <LenisWrapper>
      <div className="min-h-screen relative bg-black flex flex-col items-center justify-center px-4 py-12 overflow-hidden">
        ...
        <div className="absolute top-4 right-4 sm:top-6 sm:right-6 z-10 max-w-[60vw] sm:max-w-none"><div className="px-0 py-[2px] rounded-full bg-gradient-to-b from-neutral-500 to-neutral-800"><Button variant="secondary" className="rounded-full px-5 py-2 sm:py-3 lg:py-4 bg-neutral-900 text-white border border-neutral-800 hover:bg-neutral-800 text-[10px] sm:text-sm" onClick={() => window.open("https://aaruush.org", "_blank")}>Visit Our Website</Button></div></div>
        ...
        <Form {...form}>
          {/* ✅ FIX 2: Removed `overflow-y-auto` and `max-h` classes */}
          <form onSubmit={form.handleSubmit(onSubmit, onInvalid)} className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-6 sm:gap-x-12 mt-8 w-full max-w-5xl z-10 px-2 sm:px-0 pb-10">
            ...
            <FormField control={form.control} name="branch" render={({ field }) => ( <FormItem><FormLabel>Branch</FormLabel><div className={gradientWrapperStyle}><FormControl><Input placeholder="C-Tech, etc." className={innerFieldStyle} {...field} /></FormControl></div><FormMessage /></FormItem> )}/>
            ...
            <FormField control={form.control} name="phoneNumber" render={({ field }) => ( <FormItem><FormLabel>Phone Number</FormLabel><div className={gradientWrapperStyle}><FormControl><Input type="tel" className={innerFieldStyle} {...field} /></FormControl></div><FormMessage /></FormItem> )}/>
            <FormField control={form.control} name="whatsappNumber" render={({ field }) => ( <FormItem><FormLabel>WhatsApp Number</FormLabel><div className={gradientWrapperStyle}><FormControl><Input type="tel" className={innerFieldStyle} {...field} /></FormControl></div><FormMessage /></FormItem> )}/>
            
            {/* ✅ FIX 1: Corrected dropdown logic */}
            <FormField control={form.control} name="preference1" render={({ field }) => (
              <FormItem><FormLabel>Preference 1</FormLabel><div className={gradientWrapperStyle}>
                <Select onValueChange={field.onChange} value={field.value}>
                  ...
                  <SelectContent className={selectContentStyle}>
                    {allGroupedOptions.map(group => <SelectGroup key={group.label}><SelectLabel className="text-neutral-400 px-2 py-1.5">{group.label}</SelectLabel>{group.options.filter(opt => opt.value !== pref2).map(opt => <SelectItem key={opt.value} value={opt.value} className={selectItemStyle}>{opt.label}</SelectItem>)}</SelectGroup>)}
                  </SelectContent>
                </Select>
              </div><FormMessage />
              </FormItem>
            ...
            <FormField control={form.control} name="preference2" render={({ field }) => (
              <FormItem><FormLabel>Preference 2</FormLabel><div className={gradientWrapperStyle}>
                <Select onValueChange={field.onChange} value={field.value}>
                  ...
                  <SelectContent className={selectContentStyle}>
                    {allGroupedOptions.map(group => <SelectGroup key={group.label}><SelectLabel className="text-neutral-400 px-2 py-1.5">{group.label}</SelectLabel>{group.options.filter(opt => opt.value !== pref1).map(opt => <SelectItem key={opt.value} value={opt.value} className={selectItemStyle}>{opt.label}</SelectItem>)}</SelectGroup>)}
                  </SelectContent>
                </Select>
              </div><FormMessage />
              </FormItem>
