...
import committeesOptions from '../../data/commitees.json'; // Adjust path if needed
import domainsOptions from '../../data/domains.json';
import teamsOptions from '../../data/teams.json';

// Translated Zod schema from your previous Yup schema
...
const RecruitmentForm: React.FC = () => {
  ...
  // const { isRecruitmentTransitioning, targetUrl: recruitmentTargetUrl, navigateToRecruitment, onAnimationStart: onRecruitmentAnimationStart } = useRecruitmentNavigation();
  // const { isLogoTransitioning, targetUrl: logoTargetUrl, navigateToMain, onAnimationStart: onLogoAnimationStart } = useLogoNavigation();
...
const {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await axios.post('https://api.aaruush.org/api/v1/cfa/users', data);
      toast.success('Application submitted successfully!', { id: toastId });
      form.reset();
    } catch (error) {
      const errorMessage = axios.isAxiosError(error) && error.response?.data?.message ? error.response.data.message : 'An unexpected error occurred.';
  ...
  return (
    <LenisWrapper>
      <div className="min-h-screen relative bg-black flex flex-col items-center justify-center px-4 py-12 overflow-hidden">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-6 sm:gap-x-12 mt-8 w-full max-w-5xl z-10 px-2 sm:px-0 overflow-y-auto max-h-[calc(100vh-200px)]">
            {/* All form fields are unchanged, they will work as intended */}
            ...
            <FormField control={form.control} name="preference1" render={({ field }) => ( <FormItem><FormLabel>Preference 1</FormLabel><div className={gradientWrapperStyle}><Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger className={innerFieldStyle}><SelectValue placeholder="Select Preference 1" /></SelectTrigger></FormControl>
            <SelectContent className={selectContentStyle}>{groupedOptions.map(group => <SelectGroup key={group.label}>
              <SelectLabel className="text-neutral-400 px-2 py-1.5">{group.label}</SelectLabel>{group.options.map(opt => <SelectItem key={opt.value} value={opt.value} className={selectItemStyle}>{opt.label}</SelectItem>)}</SelectGroup>)}</SelectContent></Select></div><FormMessage /></FormItem> )}/>
            <FormField control={form.control} name="preference2" render={({ field }) => ( <FormItem><FormLabel>Preference 2</FormLabel><div className={gradientWrapperStyle}><Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger className={innerFieldStyle}><SelectValue placeholder="Select Preference 2" /></SelectTrigger></FormControl><SelectContent className={selectContentStyle}>{groupedOptions.map(group => <SelectGroup key={group.label}><SelectLabel className="text-neutral-400 px-2 py-1.5">{group.label}</SelectLabel>{group.options.map(opt => <SelectItem key={opt.value} value={opt.value} className={selectItemStyle}>{opt.label}</SelectItem>)}</SelectGroup>)}</SelectContent></Select></div><FormMessage /></FormItem> )}/>
        ...
        {isRecruitmentTransitioning && <RecruitmentPageTransition targetUrl={recruitmentTargetUrl} onAnimationStart={onRecruitmentAnimationStart} isActive={isRecruitmentTransitioning}/>}
        {isLogoTransitioning && <LogoTransition targetUrl={logoTargetUrl} onAnimationStart={onLogoAnimationStart} isActive={isLogoTransitioning}/>}
      </div>
    </LenisWrapper>
  );
};
