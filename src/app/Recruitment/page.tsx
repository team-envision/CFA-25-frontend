...
const formSchema = z.object({
  ...
  srmEmail: z.string().email().min(1).regex(/^[\w-\.]+@srmist\.edu\.in$/).trim().toLowerCase(),
  phoneNumber: z.string().min(10).max(10).regex(/^[6-9]\d{9}$/),
  whatsappNumber: z.string().min(10).max(10).regex(/^[6-9]\d{9}$/),
  preference1: z.string().min(1),
  preference2: z.string().min(1),
});
...
const RecruitmentForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setGroupedOptions([
      {
        label: "Teams",
        options: teamsOptions.filter((opt) => !selected.includes(opt.value)),
      },
      {
        label: "Committees",
        options: committeesOptions.filter((opt) => !selected.includes(opt.value)),
      },
      {
        label: "Domains",
        options: domainsOptions.filter((opt) => !selected.includes(opt.value)),
      },
    ]);
  }, [pref1, pref2]);
  ...
  const handleClick = () => navigateToRecruitment("envision_recruitment");
  const handleLogoClick = () => navigateToMain();
      ...
      <div className="min-h-screen relative bg-black flex flex-col items-center justify-center px-4 py-12 overflow-hidden">
        ...
        <div className="absolute top-4 right-4 sm:top-6 sm:right-6 z-10 max-w-[60vw] sm:max-w-none">
          <div className="px-0 py-[2px] rounded-full bg-gradient-to-b from-neutral-500 to-neutral-800">
            <Button
              variant="secondary"
              className="rounded-full px-5 py-2 sm:px-5 sm:py-3 lg:px-5 lg:py-4 bg-neutral-900 text-white border border-neutral-800 hover:bg-neutral-800 transition text-[10px] sm:text-sm md:text-base"
        ...
        <div className="w-full max-w-[780px] mt-4 z-10 px-2 sm:px-0">
          <div className="py-px rounded-2xl bg-gradient-to-b from-[#796856] to-[#574030a1]">
            <div className="bg-[#2a170b] rounded-2xl px-3 sm:px-6 py-2.5 sm:py-3.5 text-sm sm:text-md text-neutral-200 font-medium">
